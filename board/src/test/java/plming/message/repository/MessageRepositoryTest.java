...
//        List<Message> list = messageRepository.findMessageAllListByUser(user);
...
//        User user = userRepository.findById(25L).orElseThrow(()-> new CustomException(ErrorCode.USERS_NOT_FOUND));
//        User other = userRepository.findById(26L).orElseThrow(()-> new CustomException(ErrorCode.USERS_NOT_FOUND));
//        List<Message> list = messageRepository.findMessageListByUserAndOther(user,other);
//        for(Message message : list){
//            System.out.println(message.toString());
//        }
//    }
